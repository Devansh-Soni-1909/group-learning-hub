...
DEFAULT_STATE_FILE = Path(__file__).with_name("collect-iscsi-data-state.json")
...
def build_prometheus_metrics(report: dict) -> str:
    lines: List[str] = []

    # Nodes configured as iSCSI targets
    lines.append(
        "# HELP iscsi_target_node Indicates whether the node is configured as an iSCSI target"
    )
    lines.append("# TYPE iscsi_target_node gauge")
    for node in report["nodes"]:
        node = prometheus_escape(node)
        lines.append(f'iscsi_target_node{{node="{node}"}} 1')
    lines.append("")

    # Total Project Images
    lines.append(
        "# HELP iscsi_projected_images_total Total projected images per iSCSI target"
    )
    lines.append("# TYPE iscsi_projected_images_total gauge")

    for node_summary in report["nodes_summary"]:
        node = prometheus_escape(node_summary["node"])
        lines.append(
            f'iscsi_projected_images_total{{node="{node}"}} '
            f'{node_summary["total_active_images"]}'
        )
    lines.append("")

    # Rootfs Images
    lines.append("# HELP iscsi_rootfs_images_total Total projected rootfs images")
    lines.append("# TYPE iscsi_rootfs_images_total gauge")
    for node_summary in report["nodes_summary"]:
        node = prometheus_escape(node_summary["node"])
        lines.append(
            f'iscsi_rootfs_images_total{{node="{node}"}} '
            f'{node_summary["rootfs_count"]}'
        )
    lines.append("")

    # PE Images
    lines.append("# HELP iscsi_pe_images_total Total projected PE images")
    lines.append("# TYPE iscsi_pe_images_total gauge")
    for node_summary in report["nodes_summary"]:
        node = prometheus_escape(node_summary["node"])
        lines.append(
            f'iscsi_pe_images_total{{node="{node}"}} ' f'{node_summary["pe_count"]}'
        )
    lines.append("")

    # Deleted rootfs Images
    lines.append("# HELP iscsi_deleted_rootfs_images_total Total deleted rootfs images")
    lines.append("# TYPE iscsi_deleted_rootfs_images_total gauge")
    for node_summary in report["nodes_summary"]:
        node = prometheus_escape(node_summary["node"])
        lines.append(
            f'iscsi_deleted_rootfs_images_total{{node="{node}"}} '
            f'{node_summary["deleted_rootfs"]}'
        )
    lines.append("")

    # Deleted PE Images
    lines.append("# HELP iscsi_deleted_pe_images_total Total deleted PE images")
    lines.append("# TYPE iscsi_deleted_pe_images_total gauge")
    for node_summary in report["nodes_summary"]:
        node = prometheus_escape(node_summary["node"])
        lines.append(
            f'iscsi_deleted_pe_images_total{{node="{node}"}} '
            f'{node_summary["deleted_pe"]}'
        )
    lines.append("")

    # LUN level read metrics
    lines.append("# HELP iscsi_lun_read_megabytes_total Total read megabytes")
    lines.append("# TYPE iscsi_lun_read_megabytes_total counter")
    for node_summary in report["nodes_summary"]:
        for image in node_summary["images"]:
            labels = {
                "node": image["node"],
                "iqn": image["iqn"],
                "lun": image["lun_name"],
            ...
            label_string = ",".join(
                f'{key}="{prometheus_escape(str(value))}"'
                for key, value in labels.items()
            )
            lines.append(
                f"iscsi_lun_read_megabytes_total{{{label_string}}} "
                f"{image["read_mbytes"]}"
            )
    lines.append("")

    # IOPS/ SCSI command count
    lines.append("# HELP iscsi_lun_in_cmds_total Total SCSI commands")
    lines.append("# TYPE iscsi_lun_in_cmds_total counter")
    ...
    for node_summary in report["nodes_summary"]:
        for image in node_summary["images"]:
            ...
            lines.append(
                f"iscsi_lun_in_cmds_total{{{label_string}}} " f"{image["in_cmds"]}"
            )
    lines.append("")
    return "\n".join(lines)


def write_prometheus_textfile(content: str, output_path: Path) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--prometheus-textfile", help="Write Prometheus textfile collector output"
    )
    return parser
...
def main() -> int:
    ...
    for node in nodes:
        images, iqns, collection_errors = collect_node_images(node, args.base_path)
        if collection_errors:
            errors[node] = "; ".join(collection_errors)
        current_snapshots[node] = snapshot_images_by_node(images).get(node, {})
        node_results.append({"node": node, "iqns": iqns, "images": images})
    ...
    if args.json:
        ...
        write_prometheus_textfile(metrics, Path(args.prometheus_textfile))
    else:
